...
- Run `make db-sync` to start PostgreSQL and execute the complete reproducible data cycle: ingest the immutable source version, classify companies, calculate Parquet model artifacts, publish the relational score projection, regenerate the JSON bundle, and regenerate `params.json`, `entities.json`, `products/`, and `horizons/` from that exact bundle.
- `make db-sync` finishes with `xray-db verify-static-data`, which rejects missing files, incompatible schemas, entity-count drift, and mismatched bundle IDs. Entity aliases are deterministic presentation data keyed by the immutable IDs; never edit the generated JSON, derive names from scores, or put aliases in an Alembic revision.
...
- The deployment command validates the mounted bundle and all four Rumbo data families before starting a release, then requests their five public endpoints after startup. A missing or mismatched derivative fails loudly instead of falling back to technical identifiers.
