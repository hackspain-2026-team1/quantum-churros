...
45 -> 65 (improvement) and 82 -> 68 (deterioration) over 24 months move 2-3
points a quarter: the three-month verdict keeps reading them as stable and
the Theil-Sen drift is supposed to make the call at least six months before
the last month. The improvement case passes. The erosion case does not reach
the frozen long bar: 14 points over 23 months accumulate only ~7.3 inside any
twelve-month window, under ``long_threshold = 8`` — the second test pins that
gap (xfail) so lowering the bar or lengthening the window turns it green.
A one-month spike stays a bump and never becomes a drift alert.
"""
    ...
    return [
        EntityMonth(
            PanelRow(
                ...
                month=parts.month, months_observed=index + 1,
            ),
            {key: PillarResult(key=key, score=parts.pillar_scores[key], gates=()) for key in PILLAR_KEYS},
            parts, verdicts[index],
        )
...
def test_the_briefs_improvement_case_is_alerted_early_by_the_drift(score_parts, params) -> None:
    verdicts, alerts = _canonical_alert(score_parts, params, 45.0, 65.0, "improvement_structural")
    ...
    alert_index = next(
        index for index, entry in enumerate(_entity_months(score_parts, params, _ramp(45.0, 65.0)))
    ...
    assert not [
        alert for alert in build_alerts(
...
@pytest.mark.xfail(
    strict=False,
    reason=(
        "the frozen long bar misses the brief's erosion case: 82 -> 68 accumulates "
        "~7.3 points inside any twelve-month window, under long_threshold = 8; "
        "lowering the bar, lengthening the window or a fitted per-band threshold "
        "turns this green (Q-05)"
    ),
)
def test_the_briefs_erosion_case_reaches_the_bar(score_parts, params) -> None:
    verdicts, alerts = _canonical_alert(score_parts, params, 82.0, 68.0, "deterioration_structural")
    assert (verdicts[-1].direction, verdicts[-1].horizon, verdicts[-1].nature) == (
        "deteriorating", "long", "structural",
    )
    assert len(alerts) == 1 and alerts[0].state == "fired"
    alert_index = next(
        index for index, entry in enumerate(_entity_months(score_parts, params, _ramp(82.0, 68.0)))
        if entry.row.month == alerts[0].month
    )
    assert (MONTHS - 1) - alert_index >= LEAD


def test_a_one_month_spike_is_a_bump_and_never_a_drift_alert(score_parts, params) -> None:
    scores = [60.0] * MONTHS
    spike = 15
    scores[spike] = 70.0
    ...
    assert (verdicts[spike].direction, verdicts[spike].horizon, verdicts[spike].nature) == (
        "improving", "short", "shock_pending",
    )
