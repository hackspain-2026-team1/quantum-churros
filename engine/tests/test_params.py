...
def test_hand_set_values_follow_the_spec(params) -> None:
    ...
    assert (
        params.anchors["payments"].points
        == params.anchors["collections"].points
        == (
            (-30.0, 100.0),
            (-20.0, 90.0),
            (0.0, 80.0),
    ...
    assert (liquidity.outflow_window_months, liquidity.outflow_fallback_months) == (
        3,
        12,
    )
    ...
    assert (
        invoices.min_invoices,
        invoices.min_effective_n,
        invoices.stamped_share_max,
    ) == (10, 5.0, 0.5)
    activity = params.activity
    assert (
        activity.coverage_window_months,
        activity.recent_months,
        activity.prior_months,
    ) == (6, 3, 6)
    ...
    assert (params.penalty.lam, params.penalty.tau) == (
        0.5,
        40.0,
    )  # tau = the critical-band boundary
    caps = params.caps
    assert (caps.negative_liquidity_ceiling, caps.weak_payments_ceiling) == (40.0, 50.0)
    assert (
        caps.negative_liquidity_min_months,
        caps.negative_liquidity_window_months,
    ) == (3, 6)
    assert (
        caps.weak_payments_threshold == 40.0
    )  # reachable: more than 60 days beyond terms
    assert (
        params.anchors["payments"](60.0) == 40.0
        and params.anchors["payments"](90.0) < 40.0
    )
    assert dict(params.bands) == {
        "critical": 0.0,
        "watch": 40.0,
        "stable": 60.0,
        "solid": 80.0,
    ...
    assert (
        trajectory.horizon_months,
        trajectory.min_delta_points,
        trajectory.min_sigma_multiple,
    ) == (3, 6.0, 1.5)
    assert (trajectory.sigma_floor, trajectory.structural_consecutive_months) == (
        2.0,
        2,
    )
    assert (trajectory.min_scored_months, trajectory.bump_revert_months) == (6, 2)
    assert (
        trajectory.perimeter_shift_share,
        trajectory.perimeter_shift_window_months,
    ) == (0.2, 3)
    ...
    assert (
        profile.concentration_top1_share,
        profile.concentration_min_months,
        profile.concentration_window_months,
    ) == (0.2, 6, 12)
    mirror = params.mirror
    assert (
        mirror.min_amount_eur,
        mirror.max_day_gap,
        mirror.weekend_bridge_day_gap,
    ) == (100.0, 1, 3)
...
def test_the_reference_is_fitted_and_frozen(params) -> None:
    ...
    assert (
        isinstance(params.fitted_on, str) and len(params.fitted_on) == 64
    )  # dataset hash
    ...
    liquidity = params.liquidity
    assert liquidity.band_anchors_fitted is True and set(liquidity.band_anchors) == set(
        SIZE_BANDS
    )
    ...
    median_days = {
        band: liquidity.band_anchors[band].points[3][0] for band in SIZE_BANDS
    }
...
def test_segmented_switch_falls_back_to_the_absolute_curve(params, panel_rows) -> None:
    ...
    flat = dataclasses.replace(
        params, liquidity=dataclasses.replace(params.liquidity, segmented=False)
    )
    ...
    for band in (*SIZE_BANDS, None):
        ...
        expected = (
            params.liquidity.band_anchors[band] if band else params.anchors["liquidity"]
        )
...
def test_dash_rules_are_the_high_precision_table(params) -> None:
    ...
    assert all(
        rule.flow_class in FLOW_CLASSES and rule.label in DASH_LABELS
    ...
    assert (
        params.dash_rules[0].id == "retention_adjustment"
    )  # whales leave before anything else
    ...
    assert fires(
        "amortisation_charge", "CARGO POR AMORTIZACION ANTICIPADA DE [COMPANY]"
    )
    assert fires("amortisation", "AMORTIZ.PTMO [NUM]") and not fires(
        "amortisation", "COMISION AMORTIZACION"
    )
    assert fires("retention_adjustment", "AP.RET.DST: [ACCOUNT] MV#")
    assert fires("retention_adjustment", "MANUAL QUITAR RETENCION")
    assert fires("social_security", "TGSS COTIZACION") and fires(
        "social_security", "SEG.SOCIAL [NUM]"
    )
    assert fires("fee", "MONTHLY FEE") and not fires("fee", "COFFEE SHOP")
    assert (
        rules["sepa_overboeking"].sign == "positive"
        and rules["loan_instalment"].sign == "negative"
    )
...
def test_round_trip_and_strict_parsing(params) -> None:
    ...
    with pytest.raises(ParamsError, match="strictly increasing"):
        params_from_dict(
            {**data, "anchors": {**data["anchors"], "debt_burden": [[1, 0], [1, 50]]}}
        )
    ...
    with pytest.raises(ParamsError, match="flow_class"):
        rules = [dict(data["dash_rules"][0], flow_class="salary")] + data["dash_rules"][
            1:
        ]
        params_from_dict({**data, "dash_rules": rules})
    ...
    with pytest.raises(ParamsError, match="band_anchors"):
        tables = {
            key: value
            for key, value in data["liquidity"]["band_anchors"].items()
            if key != "large"
        }
        params_from_dict(
            {**data, "liquidity": {**data["liquidity"], "band_anchors": tables}}
        )
    with pytest.raises(ParamsError, match="one point per band score"):
        tables = {
            **data["liquidity"]["band_anchors"],
            "micro": [[0, 0], [10, 50], [20, 100]],
        }
        params_from_dict(
            {**data, "liquidity": {**data["liquidity"], "band_anchors": tables}}
        )
