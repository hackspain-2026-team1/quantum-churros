...
export interface FinanciacionM {
  id: string;
  kind: "factoring" | "confirming" | "line" | "restructure" | "sweep";
  title: string;
  detail: string;
  amount: number | null;
...
export interface MesM {
  ...
  /** Opcional, bundles nuevos: lo que la gestión no alcanza y un banco sí (deltas reales del motor). */
  financing?: FinanciacionM[];
}
...
export interface GrupoM extends EntidadBase {
  kind: "group";
  companies: EmpresaResumenM[];
}
...
export interface FilaEvidenciaM {
  pillar: Pilar | null;
  label: string;
...
export type ProductoId =
  ...
  | "depositos"
  | "plan_pensiones";
export interface ItemProductoM {
  bank: string | null;
  label: string | null;
  granted: number | null;
...
export interface ProductosIndiceM {
  ...
  rules: Record<
    string,
    { declared?: string; inferred?: string; precision_note?: string }
  >;
  portfolio: Record<
    string,
    { companies: number; groups: number; declared: number; inferred: number }
  >;
...
export interface ParametrosM {
  ...
  liquidity: {
    band_anchors: Record<string, Tabla>;
    month_end_weight: number;
    intra_min_weight: number;
