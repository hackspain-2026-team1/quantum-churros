...
import type {
	AlertaM, EmpresaM, EntidadesM, EvidenciaM, GrupoM, HorizonteM, HorizontesIndiceM, Manifiesto, ParametrosM,
	ProductosEmpresaM, ProductosGrupoM, ProductosIndiceM, ReciboM,
} from './contrato';

const BASE = import.meta.env.BASE_URL;
const conBarra = (r: string) => (r.endsWith('/') ? r : `${r}/`);
export const RAIZ_BUNDLE = conBarra(import.meta.env.VITE_DATOS || `${BASE}datos/`);
export const RAIZ_RUMBO = conBarra(import.meta.env.VITE_RUMBO || `${BASE}rumbo/`);
...
function leer<T>(url: string, opcional = false): Promise<T | null> {
	if (!cache.has(url)) cache.set(url, opcional ? pedir<T>(url).catch(() => null) : pedir<T>(url));
...
export async function yaCargado<T>(p: Promise<T>): Promise<T> { return p; }

/** Los datos de Rumbo tienen que ser del mismo bundle que el motor: si no, se avisa. */
export function mismaHuella(bundleId: string, otro: { bundle_id?: string } | null) {
