...
async function iniciar() {
	...
	try {
		...
		app.append(h('div', { class: 'sin-datos' }, h('h1', {}, falta ? 'Rumbo no encuentra los datos' : 'Rumbo no puede leer los datos'),
			...
			h('p', {}, 'Para prepararlos: exportar con el motor (make export) y ejecutar scripts/datos/parametros.py, productos.py y horizontes.py. Ver frontend/DIARIO.md.')));
		return;
	}
	...
	barra.append(marca, h('span', { class: 'hueco' }), lentes, selector, nota, botonFinanciacion, botonMetodo, botonAyuda);
	...
	for (const [k, d] of [['1 2 3 4', 'Scoring, productos, acciones y técnico (en una organización o una empresa)'], ['Escribe', '«se tuercen», «T2», «factoring», «42»…'], ['← →', 'Mover el intervalo un periodo'], ['⇧ ← →', 'Mover solo «desde»'], ['[ ]', 'Escala más fina o más gruesa'], ['Espacio', 'Reproducir o parar'], ['↵', 'Abrir el grupo señalado'], ['↑ ↓', 'Grupo anterior o siguiente'], ['Esc', 'Subir un nivel o quitar el último filtro'], ['⌘Z', 'Deshacer'], ['?', 'Esta ayuda']])
