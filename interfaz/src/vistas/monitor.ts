...
import type { IndiceEmpresasM, Manifiesto } from '../datos/contrato';
...
export function crearMonitor(ctx: CtxMonitor): Monitor {
	...
	let indice: IndiceEmpresasM | null = null;
	...
	const raiz = h('article', { class: 'entrada monitor' });
	...
	raiz.append(cabeza, campo, columnas, vistaSec, pie);

	// El índice de empresas trae el tamaño de cada una; se lee una vez.
	void carga.indiceEmpresas().then((ix) => { indice = ix; cache.clear(); if (est.unidad === 'empresas') pintarTodo(); });
	...
	const buscar = () => {
		...
		const hits = c.groups.filter((g) => (Number.isFinite(num) && num > 0 && Number(g.id.split('_')[1]) === num) || (q.length >= 3 && `${g.industry ?? ''} ${PAISES[g.country ?? ''] ?? ''} ${f.grupo(g.id)}`.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').includes(q))).slice(0, 6);
		const tt = t();
		for (const g of hits) {
			const m = g.meses[tt];
			const li = h('li', { role: 'option', tabindex: '0', class: 'tocable' }, h('b', {}, f.grupo(g.id)), h('span', { class: 'sub' }, [g.industry, PAISES[g.country ?? ''] ?? g.country, f.plural(g.n_companies, 'empresa', 'empresas')].filter(Boolean).join(' · ')), h('span', { class: 'res-score' }, m?.shown != null ? f.score(m.shown) : '—'));
