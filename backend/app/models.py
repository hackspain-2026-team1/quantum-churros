...
class FundingNeed(SQLModel, table=True):
    ...
    drivers_json: dict[str, Any] = Field(sa_column=Column(JSON, nullable=False))
...
class FinancingCase(SQLModel, table=True):
    ...
    amount: float
    term_months: int
    product_types_json: list[str] = Field(sa_column=Column(JSON, nullable=False))
...
class Mandate(SQLModel, table=True):
    ...
    scope_json: dict[str, Any] = Field(sa_column=Column(JSON, nullable=False))
    granted_by: str
...
class ProviderCapability(SQLModel, table=True):
    ...
    eligibility_json: dict[str, Any] = Field(sa_column=Column(JSON, nullable=False))
    terms_json: dict[str, Any] = Field(sa_column=Column(JSON, nullable=False))
...
class Opportunity(SQLModel, table=True):
    ...
    id: str = Field(primary_key=True)
    case_id: str = Field(foreign_key="financing_cases.id", unique=True, index=True)
    public_code: str = Field(unique=True, index=True)
    teaser_json: dict[str, Any] = Field(sa_column=Column(JSON, nullable=False))
